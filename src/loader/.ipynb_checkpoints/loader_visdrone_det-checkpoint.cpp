...
#include "loader/loader_visdrone_det.h"
...
LoaderVisdroneDet::LoaderVisdroneDet(const std::string& image_folder,
  ...
  : path_(image_folder)
{
  ...
  const string& subfolder_path = annotations_folder;
  ...
  for (size_t j = 0; j < annotation_files.size(); ++j) {
    ...
    images_.emplace_back(annotations);
  } // Process all annotations
  printf("Found %zu annotations from %zu images\n", num_annotations, images_.size());
}

void LoaderVisdroneDet::LoadAnnotationFile(const string& annotation_file,
...
void LoaderVisdroneDet::ShowImages() const {
...
void LoaderVisdroneDet::ComputeStatistics() const {
...
void LoaderVisdroneDet::ShowAnnotations() const {
  // Iterate over all images.
  for (size_t i = 0; i < images_.size(); ++i) {
...
void LoaderVisdroneDet::LoadImage(const size_t image_num,
                                  cv::Mat* image) const {
  ...
  const string& image_file = path_ + "/" + annotation.image_path;
...
void LoaderVisdroneDet::LoadAnnotation(const size_t image_num,
...
void LoaderVisdroneDet::ShowAnnotationsRand() const {
  while (true) {
...
void LoaderVisdroneDet::ShowAnnotationsShift() const {
