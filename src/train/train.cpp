...
#include "loader/loader_visdrone_sot.h"
...
void train_image(const LoaderVisdroneDet& image_loader,
...
int main (int argc, char *argv[]) {
  ...
  // Load the image data.
  LoaderVisdroneDet image_loader(videos_folder_imagenet, annotations_folder_imagenet);
  const std::vector<std::vector<Annotation> >& train_images = image_loader.get_images();
  printf("Total training images: %zu\n", train_images.size());

  // Load the video data.
  LoaderVisdroneSot video_loader(alov_videos_folder, alov_annotations_folder);
  const bool get_train = true;
  std::vector<Video> train_videos;
  video_loader.get_videos(get_train, &train_videos);
